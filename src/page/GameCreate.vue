...
const createGame = async () => {
  ...
  try {
    const res = await axios.post(`http://localhost:3000/tic-tac-toe/create/${useUser.user?.id}`)
...
<template>
  <main
    ...
    <div
      ...
      <div class="flex flex-col sm:flex-row gap-6 justify-center" @click="createGame">
        
        <p class="relative inline-block px-3 py-5 font-bold text-lg bg-gradient-to-r from-via-purple-500 via-pink-600 to-teal-600 rounded-full shadow-lg text-white hover:scale-105 transform transition duration-300 before:absolute before:-inset-1 before:bg-gradient-to-r  before:rounded-full">
          Играть vs Компьютер
        </p>

        <RouterLink to="/games/active"
